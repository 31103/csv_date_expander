...
function handleProcessClick(): void {
  ...
  clearStatus();
  hideDownloadArea();
  hideWarningDetailsArea(); // Added: Clear previous warning details
  processedCsvString = "";
  ...
  reader.onload = (event: ProgressEvent<FileReader>) => { // Add explicit type for event
    try {
      ...
      if (!arrayBuffer) {
        ...
        throw new Error("ファイルの読み込み中に問題が発生しました。");
      }
      ...
      try {
        ...
        if (bom[0] === 0xEF && bom[1] === 0xBB && bom[2] === 0xBF) {
          ...
          try {
            ...
            csvText = new TextDecoder("shift_jis", { fatal: true }).decode(
              arrayBuffer,
            );
        ...
        // Show user-friendly error via the UI handler
      ...
      if (processedRows.length > 0) {
        ...
        showDownloadArea();
        showStatusWithWarnings(baseMessage, warningsSummary); // Use summary for status
        showWarningDetails(detailedWarnings); // Added: Show detailed warnings list
      } else {
        // Determine why no rows were generated
        if (originalDataRows.length === 0) {
          showError("CSVファイルに処理可能なデータ行が含まれていません。");
        } else if (totalSkipped === originalDataRows.length) {
          // Updated message for all rows skipped due to warnings
          showError(
            "処理できる有効なデータがありませんでした。詳細は下の警告リストを確認してください。",
          );
          // Show warnings in status even if no rows generated
          showStatusWithWarnings("処理結果なし。", warningsSummary);
          showWarningDetails(detailedWarnings); // Added: Show detailed warnings list even on full failure
        } else {
          // This case might be less likely now, but keep a generic message
          showError(
      ...
      // Warning details are now handled by showWarningDetails
    } catch (error) {
